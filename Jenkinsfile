    ...
    stages {
        ...
        stage('Deploy MySQL to DEV') {
            steps {
                ...
                sh "docker run --name jayce-mysql --rm --network dev -v jayce-mysql-data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=${MYSQL_ROOT_LOGIN} -e MYSQL_DATABASE=db_example -d mysql:8.0"
                sh 'sleep 15'
                sh "docker exec -i jayce-mysql mysql --user=root --password=${MYSQL_ROOT_LOGIN} < script"
            }
        }
